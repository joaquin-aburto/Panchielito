function submitForm(event, formType) {
    ...
    if (formType === 'login') {
        ...
        fetch('https://panchielito.com/Login/validarUsr.php', {
            method: 'POST',
            body: datos,
        })
        .then((res) => res.json())
        .then((data) => {
            ...
            if (data === 'Admin') {
                ...
                setTimeout(() => window.location.href = "https://panchielito.com/Admin/admin.php", 4000);
            } else if (data === 'Correcto') {
                respuesta.innerHTML = '<div class="alert alert-primary" role="alert">¡Usuario aceptado... Bienvenido!</div>';
                setTimeout(() => window.location.href = "https://panchielito.com/", 4000);
            }
            else {
    ...
    if (formType === 'register') {
        ...
        fetch('https://panchielito.com/Login/register.php', {
            method: 'POST',
            body: datos,
        })
        .then((res) => res.json())
        .then((data) => {
            ...
            if (data === 'Registrado') {
                ...
                setTimeout(() => {
                    document.getElementById('login-frame').style.display = 'block'; // Mostrar el formulario de inicio de sesión
                    document.getElementById('register-frame').style.display = 'none'; // Ocultar el formulario de registro
                }, 3000);
            }
            else {
                respuesta.innerHTML = `<div class="alert alert-danger" role="alert">${data}</div>`;
            }
        })
        .catch((error) => {
            console.error('Error al realizar la petición:', error);
